...
_Центроида_ дерева --- вершина, при удалении которой, каждое дерево из полученного леса будет иметь не более, чем $abs(V)/2$ вершин. #footnote[https://usaco.guide/plat/centroid?lang=cpp#centroids]  При этом, в любом дереве существует хотя бы один центроид, но не больше двух.
...
Другими словами, граф _двусвязный_, если он не содержит точек сочленения.
...
- _Whitney's theorem_:
  Граф $G$, имеющий $n >= 3$ вершин, является двусвязным тогда и только тогда, когда любые две его вершины лежат на каком-либо общем цикле.

- _Whitney's inequality_:
  Для любого графа $G$:
...
_Теорема Менгера для ребер_: пусть $u$, $v$ --- пара несмежных вершин в $G$. Тогда _максимальное_ количество реберно внутренне непересекающихся путей $u$-$v$ равно _минимальной_ мощности $u$-$v$ реберного разреза $F$.
...
- $c : E -> R_(>=0)$  --- функция _пропускной способности_ ребра, то есть сколько потока может течь по этому ребру.
...
+ _Ограничение по пропускной способности_: $0 <= f(e) <= c(e)$. Простыми словами, поток не превышает пропускную способность.
...
_Пропускная способность_ $s$-$t$ разреза $(A, B)$ равен сумме пропускных способностей ребер $A -> B$:
$
...
Более того, когда эти условия выполняются, разрез $(A, B)$ _минимален_, то есть имеет наименьшую пропускную способность.

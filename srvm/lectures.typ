...
Пусть даны $x, (a_1, a_2, ..., a_n) in Eps^n$ такие, что:
- $x perp (a_1, a_2, ..., a_k)$, то есть
- $x perp sum_(i = 1)^n lambda_i a_i$, то есть
- $x perp L(a_1, a_2, ..., a_k)$, то есть
- $x perp$ подпространству $Eps^n$
...
#definition(title: "Ортогональное дополнение")[
  ...
  называется _ортогональным дополнением_ к подпространству $L$
]

#property(title: "Свойства ортогонального дополнения")[
  1. $L^perp$ является подпространство $Eps^n$
  2. $Eps = L xor L^perp$
  3. $dim Eps = dim L + dim L^perp$
]

#proof[
  на некст лекции. по крайней мере пункт 2
]

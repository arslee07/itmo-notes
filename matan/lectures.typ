...
= Лекция 239 (14.04.2026)

== Числовые ряды

#definition(title: "Числовой ряд")[
  _Числовым рядом_ называется выражение вида:
  $
    a_1 + a_2 + ... + a_n + ... = sum_(n=1)^(oo) a_n
  $

  $a_n$ --- _общий член_ числового ряда, причем $a_n = f(n)$.  
]

Составим _частичные суммы_ ряда:

$
  S_1 = a_1, wide S_2 = a_1 + s_2, wide S_3 = a_1 + a_2 + a_3, wide ... wide S_n = a_1 + a_2 + ... + a_n
$
...
#definition(title: "Сходящийся ряд")[
  Числовой ряд называется _сходящимся_, если существует конечный предел последовательности ${S_n}$ при $n -> oo$, то есть
  $
    lim_(n -> oo) S_n = S,
  $
...
#definition(title: "Сумма ряда")[
  Число $S$ называется _суммой ряда_. Если ряд $sum_(n=1)^(oo) a_n$ сходится и его сумма равна $S$, то может быть использована запись

  $
    sum_(n=1)^(oo) a_n = a_1 + a_2 + ... + a_n + ... = S
  $
]

#definition(title: "Расходящийся ряд")[
  Если $lim_(n -> oo) S_n = oo$ или не существует, то ряд $sum_(n=1)^(oo) a_n$ называется _расходящимся_.
]
...
$
  sum_(n=1)^(oo) a q^(n-1) wide ("или " sum_(n=0)^(oo) a q^n)
$

Отсюда:

$
  S_n &= a + a q + a q^2 + ... + a q^n \
      &= a(1 + q + q^2 + ... + q^m) \
      &= (a (1 - q^n))/(q-1) \
      &= a/(1-q) - (a q^n)/(1-q)
$
...
1. $abs(q) < 1$:
    ...
    $
      lim_(n -> oo) ( a / (1 - q) - (a q^n) / (1 - q) ) = replacement(q^n -> 0; n -> oo) = a / (1 - q) = S
    $

   То есть, при $|q| < 1$ ряд сходится и его сумма равна $display(S = a / (1 - q))$ (бесконечно убывающая геометрическая прогрессия).

2. $abs(q) > 1$:

    $
      lim_(n -> oo) ( a / (1 - q) - (a q^n) / (1 - q) ) = replacement(q^n -> oo; n -> oo) = plus.minus oo
    $
...
3. $abs(q) = 1$:
  ...
  - $q = 1$:
    ...
    $
      sum_(n=1)^(oo) a dot 1 = a + a + ... + a + ... = a (1 + 1 + ... + 1 + ...)
    $

    $S_n = a n$, следовательно, $S_n -> oo$. Ряд расходится.

  - $q = -1$:

    $
      sum_(n=1)^(oo) a (-1)^(n-1) = a - a + a - a + ...
    $

    Предела $S_n$ не существует. Ряд расходится.

Итак, геометрический ряд $display(sum_(n=1)^oo a q^(n-1))$ сходится при $abs(q) < 1$ и расходится при $abs(q) >= 1$.

== Сходимость гармонического ряда

Рассмотрим _гармонический ряд_ $display(sum_(n=1)^oo 1/n)$:

$
  S_n = 1 + 1/2 + 1/3 + 1/4 + 1/5 + ... + 1/8 + 1/9 + ... + 1/16 + ... + 1/n
$
...
$
  overline(S_n) = 1 + 1/2 + 1/4 + 1/4 + 1/8 + ... + 1/8 + 1/16 + ... + 1/16 + ... + 1/2^k
$
...
$
  &overline(S)_1 = 1, \
  &overline(S)_2 = 1 + 1/2, \
  &overline(S)_4 = 1 + 1/2 + 2 dot 1/4 = 1 + 2 dot 1/2, \
  &overline(S)_8 = 1 + 1/2 + 1/2 + 4 dot 1/8 = 1 + 3 dot 1/2, \
  &dots \
  &overline(S)_(2^k) = 1 + k dot 1/2
$

Тогда $overline(S_n) -> oo$. Поскольку $S_n >= overline(S_n)$, то и $S_n -> oo$. Следовательно, гармонический ряд расходится.

#theorem(title: "Критерий Коши сходимости числового ряда")[
  Для сходимости числового ряда $sum_(n = 1)^oo a_n$ необходимо и достаточно, чтобы:
  $
    forall epsilon > 0 : quad exists N(epsilon) : quad forall n >= N(epsilon) : quad forall p in NN : abs(a_(n+1) + a_(n+2) + ... + a_(n+p)) < epsilon
  $
]
...
#proof[
  Заметим, что
  $
    a_(n+1) + a_(n+2) + ... + a_(n+p) = S_(n+p) - S_n,
  $
  где $S_n$ --- $n$-ая частичная сумма ряда.

  Тогда, так как
  $
    forall epsilon > 0 : quad exists N(epsilon) : quad forall k, m >= N(epsilon) : quad abs(S_m - S_k) < epsilon,
  $
  где $m = n + p$ и $k = n$, то ${S_n}$ является фундаментальной и в силу критерия Коши для функциональной последовательности имеет конечный предел, то есть ряд сходится.
]

_Замечание._ Если отрицание условия критерия Коши выполняется, т.е.
$
  exists epsilon_0 > 0 : forall k in NN, quad exists n >= k, quad exists p in NN : |a_(n+1) + a_(n+2) + ... + a_(n+p)| >= epsilon_0,
$
то ряд $display(sum_(n=1)^(oo) a_n)$ расходится.

#definition(title: "Обобщенный гармонический ряд (ряд Дирихле)")[
  $
    sum_(n=1)^oo 1/n^p
  $
...
#property(title: "1")[
  1. Если ряд $sum_(n=1)^(oo) a_n$ сходится и его сумма равна $S$, то ряд $sum_(n=1)^(oo) c a_n$ (где $c$ --- произвольное число) также сходится, и его сумма равна $c S$.

  2. Если ряд $sum_(n=1)^(oo) a_n$ расходится и $c eq.not 0$, то ряд $sum_(n=1)^(oo) c a_n$ также расходится.
]

#proof[
  1. Пусть $S_n^*$ --- $n$-ая частичная сумма ряда $sum_(n=1)^oo c a_n$:
    $
      S_n^* = c a_1 + c a_2 + ... + c a_n = c (a_1 + a_2 + ... + a_n) = c S_n,
    $
    где $S_n$ — частичная сумма ряда $sum a_n$. Перейдем к пределу:
    $
      lim_(n -> oo) S_n^* = lim_(n -> oo) c S_n = c lim_(n -> oo) S_n = c S.
    $
    Следовательно, ряд сходится с суммой $c S$.

  2. От противного: пусть ряд $sum_(n=1)^oo c a_n$ сходится и имеет сумму $S_1$. Тогда:
    $
      S_1 = lim_(n -> oo) S_n^* = lim_(n -> oo) (c S_n) = c lim_(n -> oo) S_n quad==> quad lim_(n -> oo) S_n = S_1 / c quad (c eq.not 0 "по условию").
    $

    Таким образом, сумма ряда равна $S_1 / c$, значит сходится. Противоречие.
]

#property(title: [2 <convergent-series-prop-2>])[
  Если ряды $sum_(n=1)^(oo) a_n$ и $sum_(n=1)^(oo) b_n$ сходятся, и их суммы соответственно равны $S_a$ и $S_b$, то сходится и ряд $sum_(n=1)^(oo) (a_n plus.minus b_n)$, причем его сумма равна $S_a plus.minus S_b$.
]

#proof[
  Пусть $S_n^a$, $S_n^b$ и $S_n$ --- $n$-ые частичные суммы рядов $sum a_n$, $sum b_n$ и $sum (a_n plus.minus b_n)$ соответственно. Тогда:

  $
    lim_(n -> oo) S_n = lim_(n -> oo) (S_n^a plus.minus S_n^b) = lim_(n -> oo) S_n^a plus.minus lim_(n -> oo) S_n^b = S_a plus.minus S_b
  $
]

#property(title: "Следствия из (2)")[
  1. Сумма/разность сходящегося ($sum a_n$) и расходящегося ($sum b_n$) рядов есть расходящийся ряд.

  2. Сумма/разность двух расходящихся рядов может быть как сходящейся, так и расходящеймся.
]

#property(title: "(3)")[
  Если к ряду $sum_(n=1)^(oo) a_n$ добавить (или отбросить) конечное число членов, то полученный и исходный ряды сходятся или расходятся одновременно.
]

#proof[
  Пусть $S$ --- сумма отброшенных членов ряда, а $k$ --- наибольший из их номеров.
  ...
  Тогда:
  $
    sum_(n=1)^oo = overbrace(underbrace(a_1 + ... + a_k, S) + underbrace(a_(k + 1) + ... + a_n, S'_n), S_n),
  $

  Следовательно,

  $
    lim_(n -> oo) S_n = S + lim_(n->oo) S'_n
  $

  Пределы $S_n$ и $S'_n$ существуют/не существуют одновременно, то есть ряд $sum_(n=1)^oo a_n$ сходится/расходится тогда и только тогда, когда сходится ряд $sum_(n=k+1)^oo a_n$.
]

#property(title: "Следствия из (3)")[
  1. Возьмем соотношение:
    $
      sum_(n=1)^oo a_n = underbrace(sum_(k=1)^n a_n, S_n) + underbrace(sum_(k=n+1)^oo a_k, R_n)
    $

    Ряды $S_n$ и $R_n$ сходятся или расходятся одновременно.

  2. Если ряд $sum_(n=1)^oo a_n$ сходится, то $lim_(n->oo) R_n = 0$.
]
...
Пусть задан ряд $sum_(n=1)^(oo) a_n$, где $a_n >= 0$.

#theorem(title: "Необходимый признак")[
  Если ряд $sum_(n=1)^(oo) a_n$ сходится, то $ lim_(n->oo) a_n = 0 $
]

#proof[
  Пусть ряд сходится. Значит, существует конечные пределы частичных сумм:

  $
    lim_(n -> oo) S_n = S wide lim_(n -> oo) S_(n-1) = S
  $

  Отсюда $a_n = S_n - S_(n-1)$, а также

  $
    lim_(n -> oo) a_n = lim_(n -> oo) (S_n - S_(n-1)) = lim_(n -> oo) S_n - lim_(n -> oo) S_(n-1) = S - S = 0.
  $
]

_Замечание._ Обратное утверждение неверно. Например, ряд $sum_(n = 1)^oo 1/n$ расходится, но $lim_(n->oo) 1/n = 0$.

#theorem(title: "Следствие. Достаточный признак расходимости")[
  Если $display(lim_(n -> oo) a_n != 0)$, то ряд $display(sum_(n=1)^oo a_n)$ расходится. 
]

#proof[
  От противного. Пусть сходится. Тогда, по необходимому признаку, предел равен нулю. Противоречие.
]

=== Достаточные признаки

#theorem(title: "Признак сравнения")[
  Пусть для рядов $sum_(n=1)^(oo) a_n$ и $sum_(n=1)^(oo) b_n$ выполнено условие:
  ...
  Тогда:
  - если сходится $sum_(n=1)^(oo) b_n$, то сходится и $sum_(n=1)^(oo) a_n$;

  - если расходится $sum_(n=1)^(oo) a_n$, то расходится и $sum_(n=1)^(oo) b_n$.
]

#proof[
  - Пусть $A_n$ и $B_n$ --- частичные суммы рядов $sum_(n=1)^oo a_n$ и $sum_(n=1)^oo b_n$ соответственно.
    - Так как $a_n <= b_n$, то и $A_n <= B_n$.
    - Так как $sum_(n=1)^oo b_n$ сходится, то существует $lim_(n -> oo) B_n = B$.
    - Так как последовательность ${B_n}$ возрастает, то $B_n <= B$. Тогда $A_n <= B_n <= B$.
    - Так как $sum_(n=1)^oo a_n$ знакоположительный, то $A_(n + 1) >= A_n$, то есть последовательность ${A_n}$ неубывающая.
    Таким образом, ${A_n}$ ограничена и убывает, следовательно, имеет предел и ряд $sum_(n=1)^oo a_n$ сходится.

  - Пусть ряд $sum_(n=1)^oo a_n$ расходится, тогда $lim_(n -> oo) A_n = oo$. Так как $B_n >= A_n$, переходя к пределу, получаем
    $
      lim_(n -> oo) B_n >= lim_(n -> oo) A_n = oo,
    $
    то есть $lim_(n -> oo) B_n = oo$, и ряд $sum_(n=1)^oo b_n$ расходится.
]

#theorem(title: "Признак сравнения в предельной форме")[
  Если $lim_(n -> oo) a_n/b_n = A$, причем $A != 0$ и $A != oo$, то ряды $sum_(n=1)^oo a_n$ и $sum_(n=1)^oo b_n$ ведут себя одинаково, то есть сходятся и расходятся одновременно.
]
...
#proof[
  ...
  $
    forall epsilon > 0 : quad exists N(epsilon) > 0 : quad forall n > N(epsilon) : quad abs(a_n/b_n - A) < epsilon
  $

  Отсюда:

  $
    -epsilon < a_n/b_n - A < epsilon, wide A - epsilon < a_n/b_n < A + epsilon, wide (A - epsilon) b_n < a_n < (A + epsilon) b_n
  $

  Если ряд $sum_(n=1)^oo a_n$ сходится, то из неравенства $(A - epsilon) b_n < a_n$ следует, что $sum_(n=1)^oo (A-epsilon) b_n$.
]

#theorem(title: "Признак д'Аламбера (Даламбера)")[
  Если $lim_(n -> oo) a_(n+1)/a_n = l$, то:
...
#proof[
  ...
  $
    forall epsilon > 0 : quad exists N(epsilon) : quad forall n > N(epsilon) : quad abs(a_(n+1) / a_n - l) < epsilon.
  $
  Отсюда получаем:
  $
    l - epsilon < a_(n+1)/a_n < l + epsilon.
  $

  - Пусть $l < 1$. Возьмем $epsilon$ столь малым, что $l + epsilon = q < 1$. Тогда из неравенства $a_(n+1)/a_n < l + epsilon$ имеем:
    $
      a_(n + 1) < a_n q, quad n < N(epsilon).
    $
    ...
    $
      ...
      &a_4 < a_3 q < a_1 q^3 \
      &dots \
      &a_(n+1) < a_n q < a_1 q^n,
    $
    то есть члены ряда  $sum_(n=2)^oo a_n$ меньше соответствующих членов ряда $sum_(n=2)^oo q^(n-1) a_1$, который сходится (т.к. геом. ряд с $abs(q) < 1$).

    По признаку сравнения, $sum_(n=2)^oo a_n$ сходится и по свойству #link(<convergent-series-prop-2>)[(2)], ряд $sum_(n=1)^oo a_n$ сходится.

  - Пусть $l > 1$.

    Тогда $lim_(n -> oo) a_(n+1)/a_n = l > 1$ и
    $
      exists N : quad forall n > N : quad a_(n+1)/a_n > 1.
    $
    Следовательно, $a_(n+1) > a_n$, то есть последовательность членов ряда возрастающая и $lim_(n->oo) a_n != 0$. Ряд расходится по необходимому признаку.
]

#theorem(title: "Радикальный признак Коши")[
  Если $lim_(n->oo) root(n, a_n) = l$, то:
  - при $l < 1$ ряд сходится,
  - при $l > 1$ ряд расходится,
  - при $l = 1$ признак не дает ответа.
]
...
#proof[
  - Пусть $l < 1$. Выберем такое $q$, что $l < q < 1$. Тогда, начиная с некоторого номера $n$ будет выполняться неравенство $root(n, a_n) < q < 1$, из чего следует $a_n < q^n$.

    Ряд $sum_(n = 1)^oo q^(n-1)$ сходится при $abs(q) < 1$, следовательно, ряд $sum_(n=1)^oo a_n$ сходится по признаку сравнения.

  - Пусть $l > 1$. Тогда, начиная с некоторого номера $n$, будет выполняться $root(n, a_n) > 1$, откуда $a_n > 1$, то есть $lim_(n -> oo) a_n != 0$, из-за чего ряд расходится по необходимому признаку.
]

#theorem(title: "Интегральный признак Коши")[
  Пусть
  - члены ряда $sum_(n=1)^oo a_n$ положительны и монотонно не возрастают,
  - $f(x)$ --- непрерывная возрастающая на $[1; +oo]$, причем $a_n = f(n)$.
  Тогда ряд $sum_(n=1)^oo a_n$ и несобственый интегралы $integral_1^oo f(x) d x$ сходятся или расходятся одновременно.
]

#proof[
  Рассмотрим криволинейную трапецию, ограниченную сверху $f(x)$ прямыми $x = 1$, $x = n + 1$ и осью $O x$:
  #figure(
    image("assets/cauchy-int-proof.svg", height: 25%)
  )

  Ее площадь равна $integral_1^(n+1) f(x) d x$.

  Впишем и опишем данную трапецию ступенчатыми фигурами, составленными из прямоугольников с единичным шагом по основаниям $[1, 2], [2, 3], ..., [n, n+1]$ и соответствующими высотами $f(2), f(3), ..., f(n+1)$ (для вписанной) и $f(1), f(2), ..., f(n)$ (для описанной).

  - Площадь вписанной фигуры равна:
  $
    S_"впис" = f(2) + f(3) + ... + f(n+1) = S_(n+1) - f(1)
  $
  - Площадь описанной фигуры равна:
  $
    S_"опис" = f(1) + f(2) + ... + f(n) = S_n
  $

  Тогда имеем неравенства:
  $
    S_n > integral_1^(n+1) f(x) d x, wide S_(n + 1) - f(1) < integral_1^(n+1) f(x) d x, wide S_(n + 1) < f(1) + integral_1^(n + 1) f(x) d x.
  $

  - Пусть интеграл $integral_1^(+oo) f(x) d x$ сходится. Тогда существует конечный предел:
    $
      lim_(n -> oo) integral_1^(n+1) f(x) d x = A.
    $
  ...
  - Пусть интеграл $integral_1^(+oo) f(x) d x$ расходится, то есть $integral_1^(n+1) f(x) d x = oo$. Из неравенства $S_n > integral_1^(n+1) f(x) d x$ следует, что $lim_(n -> oo) S_n = oo$. Значит, ряд расходится.
]

#example[
  Исследуем ряд $sum_(n=1)^oo 1/n^p$ на сходимость.

  Для этого рассмотрим функцию $f(x) = 1/x^p > 0$. При $x >= 1$, функция непрерывна и не убывает на промежутке $[1; +oo)$, так как $ f'(x) = -1/(p x^(p+1)) < 0. $

  Положив $f(n) = 1/n^p = a_n$, установим сходимость $integral_1^(+oo) (d x)/x^p$:
...
== Полезные пределы и формулы

$ lim_(n -> oo) root(n, n) = 1, wide lim_(n -> oo) root(n, n^p) = 1 $
